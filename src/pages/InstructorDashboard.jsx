...
const InstructorDashboard = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      const getUserResponse = await axios.get(
        `${API_BASE_URL}/UserModels/${instructorId}`
      );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        instructorCourses = coursesResponse.data.filter(
          (course) =>
            String(course.instructorId || course.InstructorId) ===
            String(instructorId)
        );
      } catch (err) {
  ...
  const fetchStatistics = async (courseIds) => {
    try {
      ...
      const completionsCount = resultsResponse.data.filter((result) =>
        instructorAssessmentIds.includes(
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      for (const assessment of courseAssessments) {
        ...
        const assessmentResults = resultsResponse.data.filter(
          (result) =>
      ...
      setData((prev) => ({
        ...
        courses: prev.courses.filter(
          (c) => (c.courseId || c.CourseId) !== courseId
        ),
  ...
  const confirmDeleteAssessment = async () => {
    ...
    try {
      ...
      setData((prev) => ({
        ...prev,
        assessments: prev.assessments.filter(
          (a) => (a.assessmentId || a.AssessmentId) !== assessmentId
        ),
      }));
    ...
    <div className="instructor-dashboard">
      ...
      <div className="dashboard-welcome">
        <div className="welcome-content">
          <h1>
            Welcome back,{" "}
            <span className="instructor-name">{user.name || user.Name}</span>
          </h1>
          <p>
            Manage your courses and assessments from your instructor dashboard
          </p>
        </div>
      </div>
          ...
          <div className="dashboard-sections">
            ...
            <section className="dashboard-section">
              <div className="section-header">
                <h2>
                  <FaBook className="section-icon" /> Your Courses
                </h2>
              ...
              {data.courses.length === 0 ? (
                <div className="empty-state">
                  ...
                  <div className="empty-state-actions">
                    ...
                    <button
                      onClick={fetchData}
                      className="action-button secondary refresh-button"
                    >
                    ...
                    return (
                      <div key={courseId} className="course-card">
                        <div
                          ...
                          style={{
                            background: getBackgroundColor(courseTitle),
                          }}
                        >
            ...
            <section className="dashboard-section">
              <div className="section-header">
                <h2>
                  <FaClipboardList className="section-icon" /> Your Assessments
                </h2>
                <Link to="/assessment/create" className="section-link">
                  Create New
                </Link>
              </div>
              ...
              {data.assessments.length === 0 ? (
                <div className="empty-visustate">
                  ...
                  <div className="empty-state-actions">
                    ...
                    <button
                      onClick={fetchData}
                      className="action-button primary refresh-button"
                    >
                      <FaSync className="button-icon" /> Refresh Data
                    </button>
                  </div>
                  ...
                  <table className="assessment-table">
                    ...
                    <tbody>
                      {data.assessments.map((assessment) => {
                        ...
                        // Find related course
                        const relatedCourse = data.courses.find(
                          (c) => (c.courseId || c.CourseId) === courseId
                        );
                            ...
                            <td>
                              <div className="assessment-actions">
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleDeleteAssessment(assessmentId, title)
                                  }
                                >
                                  <FaTrashAlt />
                                </button>
          ...
          <div className="confirmation-modal">
            ...
            <div className="modal-body">
              <p>
                ...
                <strong>"{deleteModal.courseTitle}"</strong>?
              </p>
              ...
              {deleteStatus.error && (
                <div className="alert alert-error">{deleteStatus.error}</div>
              )}
            </div>
        ...
        <div className="modal-overlay">
          <div className="confirmation-modal">
            ...
            <div className="modal-body">
              <p>
                Are you sure you want to delete{" "}
                <strong>"{deleteAssessmentModal.assessmentTitle}"</strong>?
              </p>
              <p className="warning-text">
                <FaExclamationTriangle className="warning-icon" />
                This action cannot be undone and will delete all associated
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                disabled={deleteAssessmentStatus.loading}
              >
                {deleteAssessmentStatus.loading
                  ? "Deleting..."
                  : "Delete Assessment"}
              </button>
            </div>
      ...
      {showDeleteModal && (
        <div className="modal-overlay">
          <div className="confirmation-modal">
            <div className="modal-header">
              <h3>
                <FaTrashAlt className="modal-icon" /> Delete Account
              </h3>
            </div>
            <div className="modal-body">
              <p>
                Are you sure you want to delete your account? This action cannot
                be undone and will delete all your courses, assessments, and
                results.
              </p>
              {deleteError && (
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const courseIds = instructorCourses.map(
                      (c) => c.courseId || c.CourseId
                    );
                    console.log(
                      `Found ${courseIds.length} courses for this instructor`
                    );
                    ...
                    const instructorAssessments =
                      assessmentsResponse.data.filter((assessment) =>
                        courseIds.includes(
                          assessment.courseId || assessment.CourseId
                        )
                      );
                    const assessmentIds = instructorAssessments.map(
                      (a) => a.assessmentId || a.AssessmentId
                    );
                    console.log(
                      `Found ${assessmentIds.length} assessments for this instructor's courses`
                    );

                    // 3. Fetch all results for these assessments
                    console.log("Fetching assessment results...");
                    const resultsResponse = await axios.get(
                      `${API_BASE_URL}/ResultModels`
                    );
                    const assessmentResults = resultsResponse.data.filter(
                      (result) =>
                        assessmentIds.includes(
                          result.assessmentId || result.AssessmentId
                        )
                    );
                    console.log(
                      `Found ${assessmentResults.length} results to delete`
                    );
                    ...
                    for (const result of assessmentResults) {
                      const resultId = result.resultId || result.ResultId;
                      await axios.delete(
                        `${API_BASE_URL}/ResultModels/${resultId}`,
                        {
                          headers: {
                    ...
                    for (const assessmentId of assessmentIds) {
                      await axios.delete(
                        `${API_BASE_URL}/AssessmentModels/${assessmentId}`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                            "Content-Type": "application/json",
                    ...
                    for (const courseId of courseIds) {
                      await axios.delete(
                        `${API_BASE_URL}/CourseModels/${courseId}`,
                        {
                          headers: {
                            Authorization: `Bearer ${token}`,
                            "Content-Type": "application/json",
                    ...
                    setDeleteError(
                      "Failed to delete account. Please ensure all related data is deleted first."
                    );
                  } finally {
      ...
      {showPasswordModal && (
        <div className="modal-overlay">
          <div className="confirmation-modal">
            <div className="modal-header">
              <h3>
                <FaKey className="modal-icon" /> Change Password
              </h3>
            </div>
            <div className="modal-body">
