...
const AllAssessmentsPage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const assessmentsResponse = await axios.get(
          "https://backendwebappedusync-cpfbfqa7fbgvhqed.centralindia-01.azurewebsites.net/api/AssessmentModels"
        );
        const assessmentsData = assessmentsResponse.data;

        // Fetch courses for additional details
        const coursesResponse = await axios.get(
          "https://backendwebappedusync-cpfbfqa7fbgvhqed.centralindia-01.azurewebsites.net/api/CourseModels"
        );
        ...
        if (instructorIds.length > 0) {
          const instructorPromises = instructorIds.map((instructorId) =>
            axios
              .get(
                `https://backendwebappedusync-cpfbfqa7fbgvhqed.centralindia-01.azurewebsites.net/api/UserModels/${instructorId}`
              )
        ...
        const combinedAssessments = assessmentsData.map((assessment) => {
          ...
          if (assessment.questions || assessment.Questions) {
            try {
              ...
              console.warn(
                "Could not parse questions for assessment",
  ...
  useEffect(() => {
      ...
      filtered = filtered.filter((assessment) => {
        return (
          ...
          (assessment.instructorName &&
            assessment.instructorName.toLowerCase().includes(searchLower))
        );
      });
    }
    ...
    if (activeFilter !== "all") {
      filtered = filtered.filter((assessment) => {
        return (
          assessment.courseTitle.toLowerCase() === activeFilter.toLowerCase()
        );
      });
    }
          ...
          {filteredAssessments.map((assessment) => (
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: generateRandomColor(assessment.courseTitle),
                }}
              >
