...
const StudentProgressPage = () => {
  ...
  const processCachedData = (instructorId, instructorCourses) => {
    ...
    const assessmentIds = courseAssessments.map(
    ...
    const studentIds = [
      ...new Set(assessmentResults.map((r) => r.userId || r.UserId)),
    ];
    ...
    const processedResults = assessmentResults
      .map((result) => {
        ...
        const student = studentUsers.find(
          (s) => (s.userId || s.UserId) === (result.userId || result.UserId)
        );
  ...
  const fetchData = useCallback(async () => {
    ...
    if (
      ...
      const instructorCourses = dataCache.courses.filter(
        (course) =>
  ...
  useEffect(() => {
    ...
    if (selectedCourse !== "all") {
      ...
      filtered = filtered.filter(
        (result) => String(result.courseId) === String(selectedCourse)
      );
    ...
    if (selectedStudent !== "all") {
      const preFilterLength = filtered.length;
      filtered = filtered.filter(
        (result) => String(result.studentId) === String(selectedStudent)
      );
      if (preFilterLength !== filtered.length) {
        shouldUpdate = true;
      }
  ...
  const calculateStats = () => {
    ...
    const maxPossibleTotal = filteredResults.reduce(
      (sum, result) => sum + (result.maxScore || 100),
      0
    );
    ...
    const highestScorePercentage = Math.max(
      ...filteredResults.map(
        (result) => ((result.score || 0) / (result.maxScore || 100)) * 100
      )
    );
  ...
  return (
    <div className="student-progress-container">
      <div className="progress-header">
        <h1>
          <FaGraduationCap className="header-icon" /> Student Progress
        </h1>
