    ...
    const fetchAssessment = async () => {
      ...
      try {
        ...
        const courseResponse = await axios.get(
          `https://backendwebappedusync-cpfbfqa7fbgvhqed.centralindia-01.azurewebsites.net/api/CourseModels/${courseId}`
        );

        // If we have instructor ID, fetch instructor details
        const instructorId =
          courseResponse.data.instructorId || courseResponse.data.InstructorId;
        if (instructorId) {
          try {
        ...
        if (courseAssessment) {
          const parsedQuestions = JSON.parse(
            courseAssessment.Questions || courseAssessment.questions
          );
          setAssessment({
            ...courseAssessment,
            Questions: parsedQuestions,
            AssessmentId:
              courseAssessment.assessmentId || courseAssessment.AssessmentId,
    ...
    <div className="assessment-container">
      <div className="assessment-header">
        ...
        <div className="assessment-info">
          <p>
            {assessment.Questions.length} questions | Max score:{" "}
            {totalPossibleMarks}
          </p>
          {instructor && (
