...
const AZURE_API_URL =
  "https://backendwebappedusync-cpfbfqa7fbgvhqed.centralindia-01.azurewebsites.net";
  ...
  async fetchFile(url) {
    try {
      ...
      if (url.includes("/api/CourseModels/download/")) {
        fileUrl = url;
        console.log(
          "[FileService] Download endpoint detected, using URL:",
          fileUrl
        );
      }
      ...
      try {
        ...
        console.log(
          "[FileService] Response received, type:",
        ...
        console.error(
          "[FileService] Direct request failed:",
          directError.message
        );
        // Detailed error logging
        if (directError.response) {
          console.error(
            "[FileService] Response error data:",
            directError.response.status,
            directError.response.statusText
          );
        } else if (directError.request) {
          console.error(
            "[FileService] Request error (no response):",
            directError.request
          );
        } else {
          console.error(
            "[FileService] Error setting up request:",
            directError.message
          );
        }
        throw directError;
      ...
      if (!url.includes("/api/")) {
        try {
          ...
          console.log(
            "[FileService] Fallback response received, type:",
            response.headers["content-type"],
            "size:",
            response.data.size
          );
          ...
          console.error(
            "[FileService] Fallback fetch also failed:",
            fallbackError.message
          );
          // Detailed fallback error logging
          if (fallbackError.response) {
            console.error(
              "[FileService] Fallback response error:",
            ...
            console.error(
              "[FileService] Error setting up fallback request:",
              fallbackError.message
            );
          }
          throw fallbackError;
...
export const uploadFile = async (
  ...
  try {
    ...
    console.log(
      `Uploading file ${file.name} (${file.size} bytes) to ${endpoint}`
    );
